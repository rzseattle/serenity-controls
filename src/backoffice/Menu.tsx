...
export class Menu extends React.Component<IMenuProps, IMenuState> {
    ...
    public render() {
        ...
        return (
            <div className={"w-menu " + (this.state.expanded ? "w-menu-expanded" : "w-menu-collapsed")} onMouseLeave={this.handleMenuLeave.bind(this)} style={style}>
                {this.props.elements.map((el, index) => (
                    <div className="menu-section" key={index}>
                        <div className="menu-section-title" onClick={() => this.setState({ currentMenuOpened: index })} onMouseEnter={this.handleTitleEnter.bind(this, index)}>
                            <Icon name={el.icon} />
                            <span>{el.title}</span>
                        </div>
