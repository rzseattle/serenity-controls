...
export default class Tbody extends React.Component<any, any> {
    ...
    public render() {
        ...
        const packValue = (val, column, row, rowComponentRef) => {
            let templateResult = false;
            if (column.template) {
                templateResult = column.template(val, row, column, rowComponentRef);
        ...
        >
            ...
            {columns.map((column, index2) => {
                ...
                return (
                    ...
                    >
                        {packFn(row[column.field] ? row[column.field] : column.default, column, row, this)}
                    </td>
                );
            })}
        </tr>);
